AddPositiveTIM <- function(J, TIM_Dens, TIMStrength ){
  if(TIM_Dens==0){return(J)}
  TIMStrength <- (TIMStrength * sqrt(pi))/sqrt(2) # Needed as an adjustor for half-normal distribution
  
  ## 'Postive' TIMs increase the strength of the interaction with more of the modifier
  #   In the short term at least this is good for the predator and bad for the prey
  ...
  ## Have to identify with more confidence which is the predator and which is the prey
