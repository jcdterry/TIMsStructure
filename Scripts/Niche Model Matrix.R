niche.model<-function(S,C,ID){ 
  # S = the number of species, C = target connectance
  # from Klecka Plos one
  
  set.seed(ID) # Set Seed for reproducibility using web ID number
  ...
  while((C.web>1.03*C)|(C.web<0.97*C)|any(is.na(TL))|components!=1 ){
    ...
    while(length(con[con==0])>0|ProdFrac<0.1){
      beta<-(1-2*C)/(2*C)
...
# Wrapper:
niche.replicated<-function(S,C,n, IDs=NULL, TIM=FALSE){
  if(is.null(IDs)){
    result.list<-lapply(1:n,function(n){niche.model(S,C, ID=n)})
    }else{
      result.list<-lapply(IDs,function(n){niche.model(S,C, ID=n)})
  }
  return(result.list)
}